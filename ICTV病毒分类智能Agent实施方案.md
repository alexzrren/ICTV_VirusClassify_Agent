...
- `scripts/batch_classify.py`：多序列FASTA并行批量分类，提交到Agent API（默认localhost:18231），输出Excel汇总（带颜色标注：High/Medium/Low置信度、novel高亮、错误标红）+ 每条序列详细TXT报告
- 关键参数：`--parallel`并发（默认4）、`--family`科名提示（跳过BLAST识别）、`--timeout`单序列超时（默认600s）
- Excel字段：Accession / Status / Time / Cached / Family / Genus / Subgenus / Species / Confidence / Novel / Evidence（rich text，方法红色、值红粗、阈值绿色）/ Reasoning / Steps / TokIn / TokOut / Calls
- 自动重试 transient 错误（API 529 过载、网络超时），失败序列写入对应TXT而不影响其他
- TXT包含完整分类层级、置信度、ICTV criterion引用、Agent推理步骤、**所提取的5个复制酶结构域FASTA序列**
- `scripts/BATCH_USAGE.md`：完整中文使用文档
- `scripts/clear_cache.py`：缓存清理小工具，支持`--dry-run`/`--all`选项

**2. ICTV VMR accession→taxonomy权威映射数据库**

- 问题：原`corona_pud.py`通过`SELECT * FROM species WHERE species LIKE '%accession%'`反查参考序列分类——SQL逻辑根本错误（species表无accession列），导致`taxonomy={}`总为空，模型只能瞎猜
- `scripts/build_vmr_accession_db.py`：从ICTV VMR_MSL40.v2.20251013.xlsx解析21541条accession→taxonomy记录，构建`vmr_accessions`表（accession、accession_version、segment、virus_name、virus_abbrev、完整11级分类、ictv_id、genome）
- 修复`backend/tools/corona_pud.py`：改用`SELECT FROM vmr_accessions WHERE accession = ?`，每条BLAST hit都能正确获得官方ICTV分类
- 验证：EF203064→*Alphacoronavirus rhinolophi*（Rhinacovirus）、MN908947→*Betacoronavirus pandemicum*（Sarbecovirus）等全部正确

**3. Subgenus全链路支持**

- `backend/models.py`：`TaxonomyResult`新增`subgenus`字段
- `backend/agent.py`：系统提示输出JSON模板加`"subgenus"`占位、`_parse_result`提取subgenus、`_build_result_from_logs` fallback正则补抓subgenus/subfamily
- 新增"Taxonomy completeness rules"：明确告知模型仅Nidovirales三科（Coronaviridae、Tobaniviridae、Arteriviridae）有subgenus，其他科留空，避免瞎编
- 验证：参考库34科中VMR含subgenus的为：Coronaviridae(28亚属)、Tobaniviridae(18)、Arteriviridae(11)
- `scripts/batch_classify.py`：Excel新增Subgenus列、TXT TAXONOMY段输出subgenus行

**4. 提取结构域序列输出（可追溯证据）**

- 目的：每条序列分类完成后，输出实际用于p-distance/PUD计算的蛋白结构域序列，方便人工复核
- `backend/models.py`：`ClassifyResult`新增`extracted_regions: list[dict]`
- `backend/tools/corona_pud.py`：`corona_classify_pud`返回临时字段`_query_domain_sequences`（5个replicase domain：3CLpro/NiRAN/RdRp/ZBD/HEL1）
- `backend/agent.py`：`_execute_tool`新增`region_store`参数，在`extract_target_region`/`compare_query_to_reference`/`corona_pud_classify`三处捕获完整序列；从模型返回值剥离避免污染上下文
- `scripts/batch_classify.py`：TXT末尾追加`EXTRACTED DOMAIN SEQUENCES`段，FASTA格式（header `>source|region|seq_type|len=N`）

**5. 抗幻觉防御层（关键改动）**

观察到MiniMax-M2.7-highspeed和GLM-4.7偶发幻觉问题：模型在Coronaviridae分类中**虚构未在参考库中的accession**（如NC_025217 Cd15-21、EF564595 HKU3）、**伪造PUD数值**（号称PUD=0.010但根本没调corona_pud_classify）、**rank误读**（PUD=11.7%属same_subgenus却写为same_species）。

设计三层确定性兜底，按从软到硬顺序应用：

- **Layer 1 — 模型层重跑**（`_classification_incomplete()` + 继续提示）：
  - 检测`stop_reason==end_turn`时分类是否完整（family/必需工具调用/species字段）
  - Coronaviridae≥20kb未调corona_pud_classify、Flaviviridae/Paramyxoviridae/Picornaviridae/Hepeviridae/Papillomaviridae未调compare_query_to_reference时
  - 注入用户消息明确告知缺什么、要求继续；最多2次nudge避免死循环
  - 保留Agent自主性，给模型自修正机会
- **Layer 2 — 通用BLAST→VMR兜底**：
  - 若模型最终taxonomy.family为空，从`blast_and_compare`/`blast_search` top hit取accession
  - 反查`vmr_accessions`，将完整taxonomy填入final_result
  - 写入说明性reasoning：`"Top BLAST hit X (description) at Y% identity. Taxonomy assigned from VMR MSL40 record. (Generated by post-processing because the agent did not produce a complete classification.)"`
- **Layer 3 — Coronaviridae确定性覆盖**：
  - 任何最终family=Coronaviridae且序列>20kb，强制查阅`corona_pud_classify`结果
  - 若模型未调，主动运行（同时capture 5个domain序列入region_store）
  - 触发条件：model genus≠tool genus、或rank≠same_species但novel=False、或rank=same_species但species名不一致
  - 触发后用工具的VMR分类**完全覆盖** taxonomy/evidence/reasoning，并按rank自动生成novel species占位符（同subgenus / 同genus / 同subfamily 三套模板）
  - 透明告知用户："This reasoning was generated by the post-processing layer because the model's output did not match the deterministic tool result."

**6. Token消耗追踪**

- `backend/models.py`：`ClassifyResult`新增`token_usage: dict`
- `backend/agent.py`：每轮API响应累加`input_tokens`/`output_tokens`/`cache_read_input_tokens`/`cache_creation_input_tokens`/`api_calls`
- `scripts/batch_classify.py`：TXT头部输出`Tokens: input=X output=Y cache_read=Z api_calls=N`、Excel新增TokIn/TokOut/Calls三列
- 用途：评估不同模型成本、识别nudge重跑代价、统计Coronaviridae（DEmARC流水线）vs 小基因组科的token差异

**7. 取消任务与SSE显示改进（前期WIP整理）**

- `backend/main.py`：新增`POST /cancel/{job_id}`端点，从`BackgroundTasks`切到`asyncio.create_task`以支持中途取消
- `frontend/index.html`：新增Stop按钮、长文本步骤折叠展开
- `run.sh`：移除硬编码API key（之前为MiniMax调试遗留），改为`${ANTHROPIC_API_KEY:?...}`强制环境变量；恢复火山引擎GLM-4.7默认配置（`https://ark.cn-beijing.volces.com/api/coding`）
